...
This report will explore the YS predictions from IIT testing used by itself and IIT combined with different composition elements, specifically Mn (Mn) and Carbon (C).  The two models will be refered to as "IIT Only" and the "IIT + Composition".
...
tab <- tidy_iit %>% 
  ...
  knitr::kable(tab, caption = "Regression Estimates for IIT Only Model")
...
The estimates of the coefficients for this model and for comparison, the values for the IIT Only model are tabulated in Table \@ref(fig:iitpc-coefs) along with their uncertainties. The high and low confidence intervals reflect the 95% range of uncertainty for each variable.
...
## UTS Discussion {-}  
A similar model for ultimatte tensile strength (UTS) was fit using the IIT measurement with Mn and C (but no the interaction). This is because C is postively correlated to UTS unlike YS negating the necessitiy of the interaction term. The metrics are shown in Table \@ref(tab:uts). The adjusted $R^2$ shows a slightly lower value than the YS model but still reasonably good give the uncertainty in several of the parameters.  It might be possible to find other elements that are suitable predictors but that is beyond the scope of this document.
...
```{r modelsum-iit, fig.cap="IIT Only Diagnostics", fig.width=6.5}
...
```{r modelsum-comp,fig.cap = "IIT + Composition Diagnostics" , fig.width=6.5}
